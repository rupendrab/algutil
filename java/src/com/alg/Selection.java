...
public class Selection
{
    ...
    public int partition(int from, int to, PivotType pivotType)
    {
        ...
        swap(pivotPosition, from); // Switch Pivot to the first position of the
                                   // array
    ...
    public int findIth(int i, int from, int to, PivotType pivotType) throws Exception
    {
        ...
        int pivotPosition = partition(from, to, pivotType) - 1;
        if (pivotPosition - from == i)
        {
            return arr[pivotPosition];
        }
        else if (pivotPosition - from > i)
        {
            return findIth(i, from, pivotPosition, pivotType);
        }
        else
        {
            return findIth(i + from - 1 - pivotPosition, pivotPosition + 1, to, pivotType);
        }
    }
    ...
    {
        long start, end = 0;
        int[] arr = createRandomIntgerArray(15000000, 100000);
        Selection sel = new Selection(arr);
        start = getTime();
